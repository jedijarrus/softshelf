...
async def _build_install_command(pkg: dict, agent_id: str) -> str:
    ...
    if archive_type == "archive":
        ...
        parts = [
            ...
            "_sfProgress \"DBG: zipPath=$zipPath extPath=$extPath PSVersion=$($PSVersionTable.PSVersion)\"\n",
            "if (Test-Path $extPath) { _sfProgress 'DBG: extPath existiert, loesche...'; Remove-Item $extPath -Recurse -Force -ErrorAction SilentlyContinue }\n",
            "_sfProgress 'DBG: Add-Type...'\n",
            "Add-Type -AssemblyName System.IO.Compression.FileSystem -ErrorAction SilentlyContinue\n",
            "_sfProgress 'DBG: ExtractToDirectory...'\n",
            "try {\n",
            "    [System.IO.Compression.ZipFile]::ExtractToDirectory($zipPath, $extPath)\n",
            "    _sfProgress 'DBG: ExtractToDirectory OK'\n",
            "} catch {\n",
            "    _sfProgress \"DBG: .NET-Extract Fehler: $($_.Exception.Message), versuche Expand-Archive\"\n",
            "    Expand-Archive -Path $zipPath -DestinationPath $extPath -Force\n",
            "    _sfProgress 'DBG: Expand-Archive OK'\n",
            "}\n",
            "_sfProgress 'DBG: vor Join-Path entry_point'\n",
            f"$exe = Join-Path $extPath '{ep_quoted}'\n",
            '_sfProgress "DBG: exe=$exe"\n',
            "_sfProgress 'DBG: vor Test-Path'\n",
            "$exeExists = Test-Path -LiteralPath $exe\n",
            '_sfProgress "DBG: exists=$exeExists"\n',
            "if (-not $exeExists) {\n",
            ...
            '    cmd /c "exit 1"\n',
            "}\n",
            "_sfProgress 'DBG: vor Split-Path'\n",
            "$workDir = Split-Path -LiteralPath $exe -Parent\n",
            '_sfProgress "DBG: workDir=$workDir"\n',
